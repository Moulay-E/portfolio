...
import ImgTest from "../../asset/img/blueCatCuteFace.png";
...
export default function Test({arr}){
    ...
    <a href="#" 
    className="flex-shrink-0 imgContainer">
        <Image className= "myImage rounded-t-lg" src={arr.cover}
         alt="product image"
        //  layout="responsive"
         width={100} 
         height={100} 
    ...
    <div className="flex flex-col justify-around flex-grow px-5 pb-5 ">
        <a href="#">
            <h5 className=" font-semibold tracking-tight text-gray-900 dark:text-white"> {arr.title} </h5>
        </a>
       
        <div className="flex items-center justify-between self-center">
            <span className=" font-bold text-gray-900 dark:text-white">{arr.description}</span>
        </div>
        ...
        <div className="flex items-center mt-2.5 mb-5 self-center justify-around">
            <p>
                {arr.tags.map((tag, index) => (
                    <span key={index}
                    class="bg-purple-100 text-purple-800 text-xs font-medium mr-2 px-2.5 py-0.5 rounded-full dark:bg-purple-900 dark:text-purple-300">
                       {tag}
                    </span>
                ))}
            </p>
